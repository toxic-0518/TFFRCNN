...
def vis_detections(im, image_name, class_name, dets, thresh=0.5):
    ...
    for i in inds:
        ...
        cv2.rectangle(res, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), (0, 255, 0), 1)
    extend = image_name.split('.')[-1]
    image_name = image_name[0: -len(extend) - 1]
    image_name = image_name + '_%s.%s' % (class_name, extend)
